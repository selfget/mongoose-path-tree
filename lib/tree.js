...
function tree(schema, options) {
    ...
    var pathSeparator = options && options.pathSeparator || '#'
        ...
        , levelExists = options && options.levelExists;
    ...
    if (!parentExists || !pathExists || !levelExists) {
    ...
    function getLevel(path) {
        return typeof path === 'string' ? path.split(pathSeparator).length : 0;
    }
    ...
    function updateDescendents (ancestor, previousPath, next) {
        ancestor.collection.find({ path: { '$regex': '^' + previousPath + pathSeparatorRegex } }, function (err, cursor) {
            ...
            streamWorker(cursor.stream(), function streamOnData(descendent, done) {

                previousPath = '' + previousPath;
                var newPath = ancestor.path + descendent.path.substr(previousPath.length);
                ancestor.collection.update({ _id: descendent._id }, { $set: { path: newPath, level: getLevel(newPath) } }, done);
            }, {
                promises: false,
        ...
        if (onDelete == 'DELETE') {
            ...
            this.collection.find({ parent: previousParent }, function (err, cursor) {
                ...
                streamWorker(cursor.stream(), function streamOnData(doc, done) {
                        ...
                        self.collection.update({ _id: doc._id }, { $set: { parent: newParent, level: doc.level - 1 } }, done);
                }, {
                    promises: false,
                    concurrency: numWorkers
                    ...
                    self.collection.find({ path: { $regex: previousParent + pathSeparatorRegex} }, function (err, cursor) {
                        ...
                        streamWorker(subStream, function subStreamOnData(doc, done) {
                            ...
                            self.collection.update({ _id: doc._id }, { $set: { path: newPath, level: doc.level - 1 } }, done);
                        }, {
                            promises: false,
                            concurrency: numWorkers
                        }, next);
